			...
			>
				{showPrompts && currentApp?.parameters?.opening_statement ? (
					<div className="flex items-center">
						{/* 左侧展示图标 */}
						<div className="border-theme-border bg-theme-bg flex h-14 w-14 items-center justify-center rounded-[50%] border-[1px] border-solid">
							<LucideIcon
								name="bot"
								size={30}
								className="text-primary dark:text-theme-text text-3xl"
							/>
						</div>
							...
							{currentApp.parameters.suggested_questions?.length ? (
								<div className="flex w-full flex-wrap items-center">
									{currentApp.parameters.suggested_questions?.map(item => {
										return (
											<div
												className="border-desc text-theme-text hover:border-primary hover:text-primary mt-2 mr-2 cursor-pointer rounded-lg border border-solid px-2 py-0.5 text-sm"
												color="blue"
												key={item}
												onClick={() => {
