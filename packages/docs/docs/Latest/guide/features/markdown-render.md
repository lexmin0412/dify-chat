...
Dify App Hub 支持对 AI 回复的各种内容进行渲染，包括图片、视频、代码、图表，以及 Dify 自身返回的各种特殊格式。
...
以上两种标签在 Dify App Hub 中都被很好地支持，展示效果如下：
...
如果你不想展示具体的表单值 json 字符串，而是需要自定义发送的消息文本，可以按照下面的指引，在应用配置中进行配置（此配置只影响界面展示，实际提交到 Dify App Hubflow 开始节点的仍然是用户填写的表单值 json 字符串）。
